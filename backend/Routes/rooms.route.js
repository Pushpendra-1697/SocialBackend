    ...
    try {
        if (floor && type) {
            const rooms = await RoomModel.find({
                floorNumber: floor,
                roomType: { $regex: type, $options: "six" },
            });
...
roomRouter.get("/:roomId", async (req, res) => {
    ...
    try {
        ...
        res
            .status(200)
            .json({ msg: `Successfully get Room which id is ${roomId}`, room });
    } catch (err) {
...
roomRouter.post("/add-room", async (req, res) => {
    ...
    try {
        ...
        res
            ...
            .json({
                status: true,
                msg: `Room Successfully Created in ${floorNumber}th floor`,
            });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Server error" });
...
roomRouter.post("/:roomId/children", async (req, res) => {
    ...
    try {
        ...
        res
            ...
            .json({
                status: true,
                msg: `Child added in ${room.roomType} room at ${room.floorNumber}th floor`,
                room,
            });
    } catch (error) {
        console.error(error);
...
roomRouter.patch("/update/:roomId", async (req, res) => {
    ...
    try {
        ...
        res
            .status(200)
            .json({
                msg: `Successfully Updated Room Details which id is ${roomId}`,
                room: updatedRoom,
            });
    } catch (err) {
        res.status(404).json({ error: err.message });
...
roomRouter.delete("/delete/:roomId", async (req, res) => {
    ...
    try {
        ...
        return res
            .status(200)
            .json({
                msg: `Successfully Deleted the Room which id is ${roomId}`,
                room,
            });
    } catch (err) {
        res.status(404).json({ error: err.message });
